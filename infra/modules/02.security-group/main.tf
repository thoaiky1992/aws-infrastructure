...
resource "aws_security_group" "prometheus_security_group" {
  ...
  vpc_id = var.vpc_id
  dynamic "ingress" {
    for_each = local.prometheus_security_group.ingress
    content {
      from_port   = ingress.value.from_port
      to_port     = ingress.value.to_port
      protocol    = ingress.value.protocol
  ...
  dynamic "egress" {
    for_each = local.prometheus_security_group.egress
    content {
      from_port   = egress.value.from_port
      to_port     = egress.value.to_port
      protocol    = egress.value.protocol
...
resource "aws_security_group" "grafana_security_group" {
  ...
  vpc_id = var.vpc_id
  dynamic "ingress" {
    for_each = local.grafana_security_group.ingress
    content {
      from_port   = ingress.value.from_port
      to_port     = ingress.value.to_port
      protocol    = ingress.value.protocol
  ...
  dynamic "egress" {
    for_each = local.grafana_security_group.egress
    content {
      from_port   = egress.value.from_port
      to_port     = egress.value.to_port
      protocol    = egress.value.protocol
